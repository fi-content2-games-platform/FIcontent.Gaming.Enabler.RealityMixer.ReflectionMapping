Shader "Custom/VideoTextureTemplateVFShader" 
{
	Properties
	{
	...
	SubShader 
	{
		Pass 
		{
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float t = _SpecularVsDiffuseRatio;
				float3 albedoTextureColor = tex2D (_MainTex, i.uv).rgb;
				
				return float4(reflectionColor(N) * (1.0 - t) + t * (diffuseColor(N) * albedoTextureColor), 1);
			}
